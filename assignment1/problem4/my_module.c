...
*/
	memoryPointer = kzalloc(MEMORY_SIZE, GFP_KERNEL);
	return 0;
}
...
*/
	if(len > MEMORY_SIZE - *off){
		len =  MEMORY_SIZE - *off;
	}
    	if(copy_to_user(buf, memoryPointer + *off, len)){
    		return -1;
    	}
	return len;
}
...
*/
	...
	if(copy_from_user(memoryPointer + *off, buf, len)){
    		return -1;
    	}
	return len;
}
